DROP FUNCTION public.get_game_list_item_for_player;

CREATE FUNCTION public.get_game_list_item_for_player(
    IN in_player_id TEXT, IN in_game_id INTEGER,
    OUT next_player_id TEXT, OUT opponents TEXT[], OUT last_move_at TIMESTAMPTZ
) AS $$
BEGIN
    last_move_at := (
        SELECT timestamp
...
RETURNS TABLE (game_id UUID, next_player_id TEXT, opponents TEXT[], last_move_at TIMESTAMPTZ)
AS $$
BEGIN
